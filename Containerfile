...
# club's whole record. (esqlite vendors its sqlite3.c into the NIF, so the
# runtime stage needs no sqlite system library.)
#
...
# agree: the builder's RUN check below, `.github/workflows/lint.yml' (the same
# build image), `.tool-versions', and the VM the suite runs on;
# mcl_bookclub_service_tests compares all four and holds both digests. The
# image's tag names a date, not a release, so the check below refuses anything
# but 28.4.3. Move all of them on purpose, never by drift.
